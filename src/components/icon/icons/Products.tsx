import React, { FC } from 'react'
import Svg, { Path, SvgProps } from 'react-native-svg'

interface Props extends SvgProps {}

const Products: FC<Props> = (props) => {
  const color = props.color || '#B9B3C2'

  return (
    <Svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      <Path
        d="M21.41 11.58L12.41 2.58C12.05 2.22 11.55 2 11 2H4C2.9 2 2 2.9 2 4V11C2 11.55 2.22 12.05 2.59 12.42L11.59 21.42C11.95 21.78 12.45 22 13 22C13.55 22 14.05 21.78 14.41 21.41L21.41 14.41C21.78 14.05 22 13.55 22 13C22 12.45 21.77 11.94 21.41 11.58ZM13 20.01L4 11V4H11V3.99L20 12.99L13 20.01Z"
        fill={color}
      />
      <Path
        d="M6.5 8C7.32843 8 8 7.32843 8 6.5C8 5.67157 7.32843 5 6.5 5C5.67157 5 5 5.67157 5 6.5C5 7.32843 5.67157 8 6.5 8Z"
        fill={color}
      />
      <Path
        d="M8.90039 12.55C8.90039 13.12 9.13039 13.62 9.50039 14L13.0004 17.5L16.5004 14C16.8704 13.63 17.1004 13.11 17.1004 12.55C17.1004 11.42 16.1804 10.5 15.0504 10.5C14.4804 10.5 13.9704 10.73 13.6004 11.1L13.0004 11.7L12.4004 11.11C12.0304 10.73 11.5104 10.5 10.9504 10.5C9.82039 10.5 8.90039 11.42 8.90039 12.55Z"
        fill={color}
      />
    </Svg>
  )
}

export default Products
